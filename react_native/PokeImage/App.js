...
export default function App() {
  ...
  const [moveValue2, setMoveValue2] = useState();
//.const estilo = {backgroundColor: black,width: 200, height: 200};
  ...
  function setAppear(name,moves,moves2){
    setNameValue(name);
    setMoveValue(moves);
    setMoveValue2(moves2);
    styles.viewPopUp.display = 'flex';
    setAppearValue('flex');

  }

  function setDisappear(){
    styles.viewPopUp.display = 'none';
    setAppearValue('none');
  }
...
change(pokemonurl,firstGenPokemonDetails);
  ...
  return (
    ...
    <View  style={styles.container}>
      ...
      <View style = {styles.viewPopUp} onMouseLeave = {()=>{setDisappear()}}>
      <Text>{nameValue+"\n"+moveValue+"\n"+moveValue2}</Text>
       </View>
    </View>
  );
...
var styles ={
  ...
  viewPopUp: { display:appearValue, zIndex:2,position:'fixed' ,backgroundColor:'lightgreen',width: 400, height: 500
